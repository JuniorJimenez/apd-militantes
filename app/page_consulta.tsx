            ...
            <aside className="space-y-4">
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 space-y-2">
                  {/* Afiliación — solo si NO está en APD o está inactivo (sin desafiliación formal) */}
                  {(!consulta.apd.registrado || apdInactivo || apdPendiente) && (
                    <Link
                      href={`/registro?cedula=${encodeURIComponent(consulta.cedula)}`}
                      className={`block rounded-xl border px-4 py-3 text-sm ${
                        jceHabilitado && !apdPendiente
                          ? 'border-blue-200 bg-blue-50 text-[#0D3B8C] hover:bg-blue-100'
                          : 'border-gray-200 bg-gray-50 text-gray-400 pointer-events-none'
                      }`}
                    >
                      {apdPendiente ? 'Solicitud en proceso (no disponible)' : 'Continuar a afiliación'}
                    </Link>
                  )}

                  {/* Desafiliación — solo si está ACTIVO y no desafiliado formalmente */}
                  {apdAfiliado && (
                    <Link
                  ...
                  {/* Constancia de afiliación — militante ACTIVO */}
                  {apdAfiliado && (
                    <a
                      href={`/carta/${encodeURIComponent(consulta.cedula)}?tipo=afiliacion`}
                      target="_blank" rel="noreferrer"
                      className="flex items-center gap-2 rounded-xl border border-[#0D3B8C]/20 bg-blue-50 px-4 py-3 text-sm text-[#0D3B8C] hover:bg-blue-100"
                    >
                      📄 Constancia de afiliación
                    </a>
                  )}

                  {/* Constancia de desafiliación — desafiliado formalmente */}
                  {desafiliadoFormal && (
                    <a
                      href={`/carta/${encodeURIComponent(consulta.cedula)}?tipo=desafiliacion`}
                      target="_blank" rel="noreferrer"
                      className="flex items-center gap-2 rounded-xl border border-[#C8001E]/20 bg-red-50 px-4 py-3 text-sm text-[#C8001E] hover:bg-red-100"
                    >
