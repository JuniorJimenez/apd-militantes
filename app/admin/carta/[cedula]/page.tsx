...
function CartaPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/carta/${encodeURIComponent(cedula)}?tipo=${tipo}`)
      .then(r => r.json())
  ...
  if (loading) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'serif' }}>
      <p style={{ color:'#666' }}>Generando carta...</p>
    </div>
  )
  if (error || !data) return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:12, fontFamily:'serif' }}>
      <p style={{ color:'#C8001E', fontWeight:600 }}>No se puede emitir la carta</p>
      <p style={{ color:'#666', fontSize:14, maxWidth:400, textAlign:'center' }}>{error}</p>
      <button onClick={() => window.history.back()} style={{ color:'#0D3B8C', fontSize:13, textDecoration:'underline', background:'none', border:'none', cursor:'pointer' }}>Volver</button>
    </div>
  )
  ...
  return (
    <>
      <style>{`
        * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; box-sizing: border-box; }
        ...
        @media print {
          ...
          .print-wrapper { background:white !important; padding:0 !important; display:block !important; min-height:0 !important; }
          .page { box-shadow:none !important; margin:0 !important; width:100% !important; min-height:100vh !important; }
        }
        body { margin: 0; font-family: 'Times New Roman', Times, serif; background: #f5f5f5; }
        .page {
      ...
      {/* Toolbar */}
      <div className="no-print" style={{ background:'#0D3B8C', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', fontFamily:'sans-serif' }}>
        <button onClick={() => window.history.back()} style={{ color:'rgba(255,255,255,.8)', background:'none', border:'none', cursor:'pointer', fontSize:13 }}>← Volver</button>
        <span style={{ color:'white', fontWeight:600, fontSize:13 }}>Carta {esAfiliacion ? 'de Afiliación' : 'de Desafiliación'} — {militante.cedula}</span>
        <button onClick={() => window.print()} style={{ background:'white', color:'#0D3B8C', fontWeight:700, border:'none', borderRadius:8, padding:'6px 16px', cursor:'pointer', fontSize:13 }}>
          🖨 Imprimir / PDF
        </button>
      </div>
      ...
      <div className="no-print" style={{ textAlign:'center', padding:'8px 16px', background:'#e5e7eb' }}>
        <p style={{ fontSize:11, color:'#9ca3af', fontFamily:'sans-serif' }}>
          Activar <strong>«Gráficos de fondo»</strong> en las opciones de impresión para conservar los colores y escudo
        </p>
      </div>

      <div className="print-wrapper" style={{ background:'#e5e7eb', padding:'24px 16px', display:'flex', flexDirection:'column', alignItems:'center' }}>
        {/* Carta */}
        <div className="page">
          ...
          <table style={{ width:'100%', borderBottom:'2px solid #0D3B8C', paddingBottom:12, marginBottom:16 }}>
            <tbody><tr>
              <td style={{ width:70, verticalAlign:'middle' }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/logo-apd.png" alt="APD" style={{ width:80, height:80, objectFit:'contain' }}/>
              </td>
              <td style={{ verticalAlign:'middle', paddingLeft:12 }}>
                ...
                <p style={{ margin:0, fontSize:10, color:'#888', marginTop:1 }}>Fundado el 15 de diciembre de 1992</p>
              </td>
              <td style={{ textAlign:'right', verticalAlign:'middle' }}>
                <p style={{ margin:0, fontSize:10, color:'#888' }}>Ref.: {numCarta}</p>
                <p style={{ margin:0, fontSize:10, color:'#888', marginTop:2 }}>Santo Domingo, {hoy}</p>
              </td>
            </tr></tbody>
          </table>
          ...
          {esAfiliacion ? (
            <>
              ...
              <p style={{ textAlign:'justify', lineHeight:1.85, fontSize:12, marginBottom:14 }}>
                La presente certificación se expide a solicitud del/la interesado/a o de autoridad competente, para los fines legales y administrativos correspondientes, en la ciudad de Santo Domingo, Distrito Nacional, a los <strong>{hoy}</strong>.
              </p>
            </>
          ) : (
            <>
              ...
              <p style={{ textAlign:'justify', lineHeight:1.85, fontSize:12, marginBottom:14 }}>
                En consecuencia, el/la ciudadano/a mencionado/a no ostenta en la actualidad ninguna membresía, cargo ni representación en nombre de la Alianza por la Democracia. La presente certificación se expide a los fines legales y administrativos que correspondan, en la ciudad de Santo Domingo, Distrito Nacional, a los <strong>{hoy}</strong>.
              </p>
            </>
          )}
          ...
          <div style={{ marginTop:48, display:'flex', justifyContent:'center' }}>
            <div style={{ textAlign:'center', minWidth:220 }}>
              <div style={{ borderTop:'1.5px solid #333', paddingTop:8, marginTop:48 }}>
                ...
                <p style={{ margin:'4px 0 0', fontSize:10, color:'#888' }}>República Dominicana</p>
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop:24, display:'flex', justifyContent:'flex-end' }}>
            <div style={{
              ...
              border:'2.5px solid #0D3B8C', display:'flex', flexDirection:'column',
              alignItems:'center', justifyContent:'center', textAlign:'center',
              color:'#0D3B8C', padding:8,
            }}>
              ...
              <div style={{ width:24, height:1.5, background:'#0D3B8C', margin:'4px 0' }}/>
...
export default function CartaFormalPage() {
  return (
    <Suspense fallback={<div style={{ minHeight:'100vh', background:'#f5f5f5', display:'flex', alignItems:'center', justifyContent:'center' }}><p style={{ fontFamily:'serif', color:'#666' }}>Cargando...</p></div>}>
      <CartaPage/>
    </Suspense>
  )
}
